...
// timespec for time information of each event
...
ht = fs->make<TH1D>("Invariant_Mass", "Invariant Mass", 644, -.52, 2.7);
ht->GetXaxis()->SetTitle("Shit");
ht->GetYaxis()->SetTitle("Number of Time");
}
...
//     reco::TrackCollection
  ...
  iEvent.getByLabel("globalMuons", gmuons);
	...
	time_t rawtime;
	ofstream ofs;
//  to store the dimuon information
	ofs.open("time.csv",ios::app);

//This is for luminosity
	Double_t lumisi = 0.;
	Handle<LumiScalersCollection> lumi;
  	iEvent.getByLabel(edm::InputTag("scalersRawToDigi"), lumi);
	for (LumiScalersCollection::const_iterator itss = lumi->begin();itss != lumi->end(); itss++) {
		lumisi = itss->instantLumi();
	}

//This is for time collection
	Handle<Level1TriggerScalersCollection> timestamp;
  	iEvent.getByLabel(edm::InputTag("scalersRawToDigi"), timestamp);
	for (Level1TriggerScalersCollection::const_iterator its = timestamp->begin();its != timestamp->end(); its++) {
		rawtime=its->collectionTime_sec();
		horaHeaven = gmtime(&rawtime);
...
// and cut on quality of 2nd global muon candidate
            && ValidHits1 >= 12
            && PixelHits1 >= 2
            && i->normalizedChi2() < 4.0 /* && it->pt()>15 && i->pt()>15 && abs(it->eta())<2.4 && abs(i->eta())<2.4 */ ) {
...
// WHY:  in order to see the various mass peaks on linear scale
          h5->Fill(s);
          h6->Fill(s);
//	Level1TriggerScalersCollection::const_iterator itts= (Level1TriggerScalersCollection::const_iterator)it;
...
// output the information of "dimuon invariant mass, luminosity, pt of muon1, pt of muon2, eta of muon1, eta of muon2, absolutetime since 1970, date of year month and day"
	  ofs<<s<<','<<lumisi<<','<<it->pt()<<','<<i->pt()<<','<<it->eta()<<','<<i->eta()<<','<<its->collectionTime_sec()<<','<<zeitHeaven<<endl;
	  ht->Fill(log10(s),w);
